...
test_that("xts input", {
  expect_true(xts::is.xts(OBV(USGDP, USGDP)))
  expect_equal(zoo::index(OBV(USGDP, USGDP)), zoo::index(USGDP))
  
  #expect_true(xts::is.xts(OBV(AAPL$AAPL.Close, AAPL$AAPL.Volume)))
  #expect_equal(zoo::index(OBV(AAPL$AAPL.Close, AAPL$AAPL.Volume)), zoo::index(AAPL))
})
